...
for epoch in range(EPOCH):
    ...
    for step, (b_x, b_y) in enumerate(train_loader):
        ...
        if step % 100 == 0:  # 每100个批次测试一次
            ...
            print(f'轮次: {epoch}, 批次: {step}')
            print(f'当前批次训练损失: {current_train_loss:.4f}, 当前批次训练准确率: {current_train_accuracy:.4f}')
    ...
    print(f'Epoch {epoch} 结束 - 平均训练损失: {epoch_avg_loss:.4f}, 平均训练准确率: {epoch_accuracy:.4f}')
...
with torch.no_grad():
    ...
    for i, idx in enumerate(sample_indices):
        true_label = "正面" if sample_labels[i] == 1 else "负面"
        pred_label = "正面" if sample_preds[i] == 1 else "负面"
        print(f"文本: {sample_texts[i][:100]}...")
